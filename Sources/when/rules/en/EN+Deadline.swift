...
extension EN {
    public static func buildDeadline(strategy: Strategy = .override) -> Rule {
        return RegexRule(
            calendar: calendar,
            regex: regex,
            applier: { match, context, _, reference in
                // 0. parse captures
                let numStr = match.captures[1].trimmingCharacters(in: .whitespaces)
                let num: Int? = try {
                ...
                if let num = num {
                    ...
                    case let string where string.contains("month"):
                        updated.dateOffset = DateOffset(days: 7 * 2)
                    case let string where string.contains("year"):
    ...
    private static let regex = try! NSRegularExpression(pattern: "(?i)(?:\\W|^)(within|in)\\s*" +
